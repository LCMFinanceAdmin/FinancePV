      ...
      <Page size="A4" orientation="landscape" style={s.page}>
        ...
        <View style={s.tbl}>
          ...
          <View style={s.thr}>
            ...
            <Text style={[s.th,{width:C.gr}]}>Gross</Text>
            <Text style={[s.th,{width:C.pcb}]}>PCB</Text>
            ...
            {customCols.map(col=><Text key={col.label} style={[col.type==="allowance"?s.thG:s.thR,{width:C.cu}]}>{col.label}</Text>)}
            <Text style={[s.th,{width:C.net}]}>Net</Text>
            <Text style={[s.th,{width:C.lcm,...noRight}]}>Total LCM</Text>
          </View>
          ...
          {monthLines.map((l,i)=>{
            ...
            return (
              <View key={i} style={row}>
                ...
                <Text style={[s.td,{width:C.gr}]}>{n(l.gross)}</Text>
                <Text style={[s.td,{width:C.pcb}]}>{n(pcbArr[i]||0)}</Text>
                ...
                {customCols.map(col=>{ const a=cAmt(mo,col); return <Text key={col.label} style={[col.type==="allowance"?s.tdG:s.tdRed,{width:C.cu}]}>{a!==0?(col.type==="allowance"?"+":"−")+n(a):"—"}</Text>; })}
                <Text style={[s.tdB,{width:C.net}]}>{n(l.net)}</Text>
                <Text style={[s.tdBl,{width:C.lcm,...noRight}]}>{n(l.totalLcmPayment)}</Text>
              </View>
            );
          })}
          ...
          <View style={s.trSub}>
            ...
            <Text style={[s.tdSub,{width:C.gr}]}>{n(monthLines.reduce((s,l)=>s+l.gross,0))}</Text>
            <Text style={[s.tdSub,{width:C.pcb}]}>{n(monthLines.reduce((s,l)=>s+l.pcb,0))}</Text>
            ...
            {customCols.map(col=>{ const t=cSub(col); return <Text key={col.label} style={[col.type==="allowance"?s.tdSubG:s.tdSubR,{width:C.cu}]}>{t!==0?(col.type==="allowance"?"+":"−")+n(t):"—"}</Text>; })}
            <Text style={[s.tdSub,{width:C.net}]}>{n(monthLines.reduce((s,l)=>s+l.net,0))}</Text>
            <Text style={[s.tdSub,{width:C.lcm,...noRight}]}>{n(monthLines.reduce((s,l)=>s+l.totalLcmPayment,0))}</Text>
          </View>
          ...
          {thirteenth ? (
            <View style={monthLines.length%2===0?s.tr:s.trAlt}>
              ...
              <Text style={[s.td,{width:C.gr}]}>{n(thirteenth.gross)}</Text>
              <Text style={[s.td,{width:C.pcb}]}>{n(pcbArr[12]||0)}</Text>
              ...
              {customCols.map(col=>{ const a=cAmt(13,col); return <Text key={col.label} style={[col.type==="allowance"?s.tdG:s.tdRed,{width:C.cu}]}>{a!==0?(col.type==="allowance"?"+":"−")+n(a):"—"}</Text>; })}
              <Text style={[s.tdB,{width:C.net}]}>{n(thirteenth.net)}</Text>
              <Text style={[s.tdBl,{width:C.lcm,...noRight}]}>{n(thirteenth.totalLcmPayment)}</Text>
            </View>
          ) : (
          ...
          <View style={s.trAnn}>
            ...
            <Text style={[s.tdAnn,{width:C.gr}]}>{n(sum(l=>l.gross))}</Text>
            <Text style={[s.tdAnn,{width:C.pcb}]}>{n(sum(l=>l.pcb))}</Text>
            ...
            {customCols.map(col=>{ const t=cTotal(col); return <Text key={col.label} style={[col.type==="allowance"?s.tdAnnG:s.tdAnnRed,{width:C.cu}]}>{t!==0?(col.type==="allowance"?"+":"−")+n(t):"—"}</Text>; })}
            <Text style={[s.tdAnn,{width:C.net}]}>{n(sum(l=>l.net))}</Text>
            <Text style={[s.tdAnn,{width:C.lcm,...noRight}]}>{n(sum(l=>l.totalLcmPayment))}</Text>
          </View>
        </View>
