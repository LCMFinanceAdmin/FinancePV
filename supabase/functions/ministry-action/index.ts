...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!["APPROVED", "REJECTED", "CHECKED"].includes(action)) {
    ...
    //
    // The checker runs the ministry's spending for the EXCO Member. They
    // usually raise the voucher themselves, and then the check is already
    // theirs; when Finance raises one on their behalf it comes here first,
    // because they are the person who knows whether the transaction is what
    // it says it is.
    //
    // Confirming the particulars is not the same as approving the payment.
    // The voucher goes back to the EXCO Member for that — unless they have
    // delegated this budget line to the checker, in which case the one act
    // does both, exactly as it does for the Education Desk today.
    if (action === "CHECKED") {
      ...
      if (pv.status !== "PENDING_CHECK") {
        return json({ error: "This voucher is not waiting on a check." }, 400);
      }
      ...
      // Has the EXCO Member delegated this budget line to them?
      //
      // If so their word settles it: the delegation exists precisely so that
      // somebody running a desk can clear its own spending, and asking the
      // EXCO to sign again for a line they have already handed over adds a
      // signature that says nothing new. If not, the check is a check — the
      // figures are right — and the voucher goes back for the EXCO to decide
      // whether the church should pay, which was never the checker's call.
      const { allowed: settlesIt } = await mayVerifyFor(
      ...
      await db.from("pvs").update({
        ...
        ...(settlesIt ? {
          head_verified: "YES",
          ministry_verified: "YES",
          ministry_verified_by: profile.full_name || user.email,
          ministry_verified_at: now,
        } : {}),
        approvals: [...(pv.approvals ?? []), {
          role: "MINISTRY_CHECKER", email: user.email,
          name: profile.full_name || user.email,
          action: settlesIt ? "CHECKED_AND_VERIFIED" : "CHECKED",
          timestamp: now,
          remarks: [
            settlesIt ? `Verified for ${pv.ministry} under a delegation for this budget line` : "",
            remarks || "",
          ].filter(Boolean).join(" — "),
          ...(settlesIt ? { delegated: true } : {}),
        }],
        updated_at: now,
      }).eq("id", pv_id);
